...
export class IngestService {
  ...
  async processMessage(dto: ExpenseSubmitDto): Promise<ProcessMessageResult> {
    ...
    this.logger.log('Processing ingest message', { fileId, fileName, customerId });
    ...
    try {
      ...
      if (existingDocument) {
        this.logger.warn('Duplicate file rejected', { fileId, documentId: existingDocument.id });
        return {
          success: true,
          duplicate: true,
      ...
      try {
        ...
        this.logger.error('Failed to create job record', recordError);
        // Non-fatal - continue even if audit record fails
      }

      this.logger.log('Created workflow job', {
        jobId: job.id,
        documentId,
        fileId,
        customerId,
      ...
      this.logger.error('Failed to process ingest message', error, { fileId });
      return {
        success: false,
        duplicate: false,
