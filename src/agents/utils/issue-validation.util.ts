 ...
 */
export function validateIssuesAgainstCompliance(issues: any[], complianceData: any, logger?: Logger): ValidationResult {
  ...
  const invalidIssues: Array<{ issue: any; reason: string }> = [];
  ...
  for (const issue of issues) {
    ...
    if (!reference || reference.trim().length === 0) {
      ...
      if (logger) {
        logger.debug(`Validation: INVALID - Empty reference for issue: ${issue.description}`);
      }
      continue;
    }
    ...
    if (isValid) {
      ...
      if (logger) {
        logger.debug(`Validation: VALID - Found match for: ${reference.substring(0, 60)}...`);
      }
    } else {
      invalidIssues.push({
        issue,
 ...
 */
function extractComplianceText(complianceData: any): string[] {
  const texts: string[] = [];
 ...
 */
export function getComplianceStats(complianceData: any): {
